...
# census_api_key("e25038dec5241a7027fd99ee907a23158898bc4c", install=TRUE)
...
#### CONGRESSIONAL DISTRICTS - FOR HOUSE RACES ####
...
#Getting data for every CD in all states in the US ####

head(fips_codes) #built into tidycensus
us <- unique(fips_codes$state)[1:51]

# alldistricts <- map_df(us, function(x) {
#   get_acs(geography = "congressional district", variables = c(currentchoice), 
#           state = x)
# })
# 
# head(alldistricts)
...
dist <- split[ ,1]
dist <- str_trim(str_replace(dist, "\\(115th Congress\\)",""))

st <- split[ ,2]
st <- str_trim(st)

alldistricts_wide$dist.name <- dist
alldistricts_wide$state.name <- st

#bring new columns forward
alldistricts_wide <- alldistricts_wide %>% 
  select(GEOID, state.name, dist.name, everything(), -NAME)
